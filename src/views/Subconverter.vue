...
export default {
  data() {
    return {
      ...
      options: {
        ...
        customBackend: {
          "私有增强型后端【vless reality+hy1+hy2+Singbox】": "https://sub.imeibeauty.com",
        ...
        remoteConfig: [
          {
            ...
            options: [
              {
                label: "默认",
                value: "https://raw.githubusercontent.com/zptpower1/nash/myini/mymaying.ini"
              },
              {
                label: "默认(私有定制)",
                value: "https://raw.githubusercontent.com/zptpower1/nash/myini/mymaying.ini"
  ...
  methods: {
    ...
    makeShortUrl() {
      ...
      let data = new FormData();// 默认使用 FormData
      let headers = {
        ...
        .then(res => {
          if (this.form.shortType === "v9.gg") {
            ...
            if (res.data.status === 200 && res.data.key !== "") {
              this.customShortSubUrl = "https://v9.gg/" + res.data.key; // 构建短链 URL
              this.$copyText("https://v9.gg/" + res.data.key); // 复制短链 URL
              this.$message.success(
                "短链接已复制到剪贴板（IOS设备和Safari浏览器不支持自动复制API，需手动点击复制按钮）"
              );
            } else {
              this.$message.error("短链接获取失败：" + res.data.error); // 显示错误信息
            }
          }
          else {
